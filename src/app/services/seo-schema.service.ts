...
export class SeoSchemaService {
  ...
  generateProductSchema(product: ProductSchemaData): void {
    ...
    const schema = {
      ...
      offers: {
        '@type': 'Offer',
        url: product.slug 
          ? `https://cmkhomeservices.com/productos/${product.slug}` 
          : 'https://cmkhomeservices.com/productos',
  ...
  generateArticleSchema(article: {
    ...
    const schema = {
      ...
      publisher: {
        ...
        logo: {
          '@type': 'ImageObject',
          url: 'https://cmkhomeservices.com/Logo Clear.png'
        }
      },
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `https://cmkhomeservices.com/blog/${article.slug}`
      }
    };
  ...
  setupProductPageSEO(product: ProductSchemaData, breadcrumbs?: { name: string; url: string }[]): void {
    ...
    // Set canonical URL
    if (product.slug) {
      this.setCanonicalUrl(`https://cmkhomeservices.com/productos/${product.slug}`);
    }
